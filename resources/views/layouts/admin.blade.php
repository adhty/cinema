        ...
        <aside class="w-64 bg-gray-900 text-gray-100 flex-shrink-0 flex flex-col p-5">
            ...
            <nav class="flex-1 space-y-2">
                ...
                <details class="group" {{ $userMenuActive ? 'open' : '' }}>
                    <summary class="flex justify-between items-center px-3 py-2 rounded-lg cursor-pointer hover:bg-yellow-400 hover:text-gray-900 transition {{ $userMenuActive ? 'bg-yellow-400 text-gray-900 font-semibold' : '' }}">
                        <span class="flex items-center"><i class="fa-solid fa-users w-5 me-2"></i> Manajemen User</span>
                        <i class="fa-solid fa-chevron-down text-xs transform group-open:rotate-180 transition"></i>
                    </summary>
                    <div class="ml-5 mt-2 space-y-1">
                        <a href="#" class="flex items-center px-3 py-1 rounded-lg transition {{ request()->routeIs('admin.roles.*') ? 'bg-yellow-400 text-gray-900 font-semibold' : 'hover:bg-yellow-400 hover:text-gray-900' }}">
                            <i class="fa-solid fa-key w-4 me-2"></i> Roles
                        </a>
                        <a href="{{ route('admin.users.index') }}" class="flex items-center px-3 py-1 rounded-lg transition {{ request()->routeIs('admin.users.*') ? 'bg-yellow-400 text-gray-900 font-semibold' : 'hover:bg-yellow-400 hover:text-gray-900' }}">
