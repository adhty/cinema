...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold text-gray-800">Edit Promo</h1>
    ...
    <div class="bg-white shadow rounded-lg p-6">
        <form action="{{ route('admin.promos.update', $promo) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                @if($promo->cover)
                    <img id="coverPreview" src="{{ asset('storage/'.$promo->cover) }}" alt="{{ $promo->title }}"
                         class="mt-2 w-32 h-24 object-cover rounded-md">
                @else
                    <img id="coverPreview" class="mt-2 w-32 h-24 object-cover rounded-md hidden">
                @endif
            </div>
            ...
            <div>
                ...
                <input type="date" id="deadline" name="deadline" value="{{ old('deadline', $promo->deadline->format('Y-m-d')) }}" required
                       class="block w-full border border-gray-300 rounded-md p-2">
            </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          class="block w-full border border-gray-300 rounded-md p-2">{{ old('description', $promo->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea id="term_condition" name="term_condition" rows="3"
                          class="block w-full border border-gray-300 rounded-md p-2">{{ old('term_condition', $promo->term_condition) }}</textarea>
            </div>

            {{-- Buttons --}}
            <div class="flex space-x-2">
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">Update Promo</button>
